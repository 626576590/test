...
import com.els.common.vo.ElsJkFunExport;
import com.els.common.vo.ElsJkFunExportKey;
...
public interface ElsJkFunExportMapper {
    @Delete({
        "delete from ELS_JK_FUN_EXPORT",
        "where JK_ID = #{jkId,jdbcType=VARCHAR}",
          "and FUN_PARAM = #{funParam,jdbcType=VARCHAR}"
    })
    int deleteByPrimaryKey(ElsJkFunExportKey key);

    @Insert({
        "insert into ELS_JK_FUN_EXPORT (JK_ID, FUN_PARAM, ",
        "FUN_PARAM_TYPE, FBK1, ",
        "FBK2, FBK3, FBK4, ",
        "FBK5, FBK6, FBK7, ",
        "FBK8, FBK9, FBK10)",
        "values (#{jkId,jdbcType=VARCHAR}, #{funParam,jdbcType=VARCHAR}, ",
        "#{funParamType,jdbcType=VARCHAR}, #{fbk1,jdbcType=VARCHAR}, ",
    ...
    int insert(ElsJkFunExport record);

    int insertSelective(ElsJkFunExport record);

    @Select({
        "select",
        "JK_ID, FUN_PARAM, FUN_PARAM_TYPE, FBK1, FBK2, FBK3, FBK4, FBK5, FBK6, FBK7, ",
        "FBK8, FBK9, FBK10",
        "from ELS_JK_FUN_EXPORT",
        "where JK_ID = #{jkId,jdbcType=VARCHAR}",
          "and FUN_PARAM = #{funParam,jdbcType=VARCHAR}"
    })
    @ResultMap("BaseResultMap")
    ElsJkFunExport selectByPrimaryKey(ElsJkFunExportKey key);

    int updateByPrimaryKeySelective(ElsJkFunExport record);

    @Update({
        "update ELS_JK_FUN_EXPORT",
    ...
    int updateByPrimaryKey(ElsJkFunExport record);
}